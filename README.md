...
安卓软件，免费，开源，无需联网，体积很小（3.7MB）。
在手机上填写客户与单据信息、勾选商品，生成销售单据的高清图片，即可保存或分享给微信好友。

界面采用 **Miuix** 重构：无缝设置列表、平滑圆角、底部导航毛玻璃、深浅色主题，清爽且现代化。
...
- **更多**：客户电话拨打，深色 / 浅色主题。
- **主题**：Monet 动态取色默认开启（品牌紫 #9c11e1，与 Logo
  一致），可切换跟随壁纸或预置主题色；底部导航为悬浮毛玻璃（平滑圆角 + 边缘高光）。
...
- Kotlin + Jetpack Compose + Miuix（组件库）
- Material 3 仅保留用于日期选择器等 Miuix 暂无替代的控件
- 图标使用 Miuix 图标集
- Room（SQLite）数据存储
